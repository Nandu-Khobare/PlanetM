...
namespace PlanetM_Ops
{
    public class PrintDataGridView
    {
        ...
        private static string[] BigColumns = { "MovieName", "Title" };
        private static string[] VBigColumns = { "Genre" };
        private static string[] LBigColumns = { "ImdbID","ReleaseDate", "Language" };
        ...
        public static void Print_DataGridView(DataGridView dgv1)
        {
            ...
            try
            {
                ...
                PaperSize psize = new PaperSize("PlanetMPaper", 1400, 2000);
            ...
            try
            {
                ...
                if (PageNo == 1)
                {
                    foreach (DataGridViewColumn GridCol in dgv.Columns)
                    {
                        ...
                        tmpWidth += 50;
                        if (BigColumns.Contains(GridCol.Name))
                            tmpWidth += 250;
                        if (LBigColumns.Contains(GridCol.Name))
                            tmpWidth += 20;
                        if (VBigColumns.Contains(GridCol.Name))
                            tmpWidth += 280;
                ...
                while (RowPos <= dgv.Rows.Count - 1)
                {
                    ...
                    else
                    {
                        ...
                        foreach (DataGridViewCell Cel in GridRow.Cells)
                        {
                            ...
                            {
                                if (Cel.OwningColumn.HeaderText.Contains("Size"))
                                {
                                    e.Graphics.DrawString(Convert.ToInt16(Cel.Value).ToString(), Cel.InheritedStyle.Font,
